...
杜振训可在任何群发送 `打开ai` / `启动ai` 或 `停止ai` / `关闭ai` 管理当前群的监听规则：前一组将“当前群 + 杜振训”加入 `targets`，后一组移除这一对规则。命令优先于普通监听规则处理，因此未监听的群也可直接发送启动命令；除全群 Stream 外，程序还会定期按杜振训的全会话消息补偿命令，确保当前账号自发的群消息不会漏处理。两种结果均会由配置的卡片机器人在原群发送确认卡片。停止命令会清空尚未开始的该群消息批次，但不会中断已经执行中的 Codex 调用。
