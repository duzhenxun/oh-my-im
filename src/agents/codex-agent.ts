...
export function runCodex(
  ...
  return new Promise((resolve, reject) => {
    ...
    callbacks.onAbortReady?.(() => {
      if (!completed) {
        // SIGTERM normally makes Codex exit with code 143. This is expected
        // when the user pauses a task, not an Agent failure.
        abortedByUser = true;
        child.kill("SIGTERM");
    ...
    child.on("error", (err) => {
      ...
      if (abortedByUser) {
        ...
        resolve({ sessionId: nextSessionId, text: "", toolStats, durationMs: Date.now() - start });
        return;
      }
      completed = true;
      clearTimeout(timeout);
    ...
    child.on("close", (code) => {
      ...
      clearTimeout(timeout);
      if (abortedByUser) {
        log.info(`aborted by user${code ? ` (exit code ${code})` : ""}`);
        resolve({ sessionId: nextSessionId, text: "", toolStats, durationMs: Date.now() - start });
        return;
      }
      if (code && code !== 0) {
