...
function chinaTimestamp(date = new Date()): string {
  // China Standard Time is UTC+8 and has no daylight-saving transitions.
  // Format explicitly instead of using toISOString(), which always emits UTC.
  const china = new Date(date.getTime() + 8 * 60 * 60 * 1_000);
  return `${china.toISOString().slice(0, -1)}+08:00`;
}
...
export function createLogger(scope: string): Logger {
  ...
  function emit(level: LogLevel, message: string, ...args: unknown[]): void {
    ...
    const prefix = `[${chinaTimestamp()}] [${scope}] [${level.toUpperCase()}]`;
