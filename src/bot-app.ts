  ...
  async function handleMessage(message: DingTalkTextMessage, replay = false): Promise<void> {
    ...
    try {
      ...
      if (state.paused) {
        ...
        if (responseMode === "card") await bot.updateReply(reply, finishedTitle("🔴", "处理暂停"), latestCardContent);
        else await bot.sendText(message.conversationId, "Agent 任务已暂停。");
        await appendConversationLog({
          id: `${message.conversationId}:${taskStartedAt}`,
        ...
        if (responseMode === "card") await bot.updateReply(reply, finishedTitle("❌", "处理失败"), `${label} 执行失败：${errorMessage}`);
        else await bot.sendText(message.conversationId, `${label} 执行失败：${errorMessage}`);
        await appendConversationLog({
          id: `${message.conversationId}:${taskStartedAt}`,
          createdAt: new Date().toISOString(),
          conversationType: "personal",
