...
function notifyGroupFailure(groupId: string, context: string, err: unknown, config?: DashboardConfig): void {
  ...
  // Failure notifications must not be sent through the current DWS login
  // account. That account is a human user and the notification could be
  // mistaken for a normal user message or be fed back into the listener.
  // Use only the explicitly configured robot webhook for failure alerts.
  const webhook = config?.webhookUrl?.trim();
  if (webhook) void sendRobotWebhookText(webhook, message)
    .catch((notifyErr) => log.error(`webhook failure notification failed group=${groupId}: ${String(notifyErr)}`));
}

async function sendRobotText(openConversationId: string, config: DashboardConfig, content: string): Promise<void> {
  // The destination must always be the openConversationId extracted from the
  // event currently being handled. The group name is only display metadata.
...
async function loadGroupSessions(): Promise<Map<string, string>> {
  try {
    const value = JSON.parse(await readFile(GROUP_SESSIONS_FILE, "utf8")) as Record<string, unknown>;
    return new Map(Object.entries(value).filter((entry): entry is [string, string] => typeof entry[1] === "string" && entry[1].trim().length > 0));
  } catch {
    return new Map();
  }
}

async function saveGroupSessions(sessions: Map<string, string>): Promise<void> {
...
function eventGroupId(event: DwsMessageEvent): string | undefined {
  // All group-side APIs that send/retrieve data expect the DingTalk
  // openConversationId. Prefer that explicit field whenever DWS provides it;
  // conversationId/conversation_id are the flattened aliases used by DWS
  // event/history payloads and carry the same openConversationId value.
  return event.openConversationId?.trim() || event.conversationId?.trim() || event.conversation_id?.trim();
}
  ...
  try {
    ...
    if (code === "EEXIST") {
      ...
      try {
        ...
        throw new Error(`group-worker is already running (lock: ${LISTENER_LOCK_FILE})`);
      } catch (lockErr) {
        if (lockErr instanceof Error && lockErr.message.startsWith("group-worker")) throw lockErr;
...
function normalizeDashboardConfig(parsed: DashboardConfig): DashboardConfig {
  ...
  return {
    ...
    cardUpdateIntervalMs: Number.isFinite(parsed.cardUpdateIntervalMs)
      ? parsed.cardUpdateIntervalMs <= 0 ? 0 : Math.max(500, Math.min(60_000, parsed.cardUpdateIntervalMs))
      : 3_000,
    showElapsed: parsed.showElapsed !== false,
    personalHistoryMessageLimit: Number.isInteger(parsed.personalHistoryMessageLimit) ? Math.max(1, Math.min(500, parsed.personalHistoryMessageLimit)) : 10,
    personalHistoryPollIntervalSeconds: Number.isFinite(parsed.personalHistoryPollIntervalSeconds) ? Math.max(0, Math.min(3600, parsed.personalHistoryPollIntervalSeconds)) : 15,
    personalHistoryLookbackMinutes: Number.isInteger(parsed.personalHistoryLookbackMinutes) ? Math.max(1, Math.min(60, parsed.personalHistoryLookbackMinutes)) : 10,
    targets,
...
async function handleMentionMonitorCommand(
  ...
  log.info(`mention monitor command=${command} group=${groupId} operator=${getSenderId(event)} target=${target.senderId} changed=${result.changed} replied=${sendReply}`);
}

function parseBindingCommand(content: string, keywords: DashboardConfig["commandKeywords"]): "open" | "stop" | undefined {
  const normalized = content.replace(/@[^\x00-\x1f\s]+/gu, "").replace(/\s+/g, "").toLowerCase();
  const matches = (key: "monitorOpen" | "monitorStop") => (keywords[key] ?? []).some((keyword) => {
    const value = keyword.replace(/\s+/g, "").toLowerCase();
    return value && (normalized === value || normalized.includes(value));
  });
  if (matches("monitorOpen") === matches("monitorStop")) return undefined;
  return matches("monitorOpen") ? "open" : "stop";
}
...
function isIgnoredRobotEvent(event: DwsMessageEvent, config: DashboardConfig): boolean {
  ...
  const senderDisplay = senderDisplayName(event).trim().toLowerCase();
  const isRobot = robotMarker(sender) ||
    (configuredRobotId && senderId === configuredRobotId) ||
    (robotName && (senderDisplay === robotName || senderValues.some((value) => value.includes(robotName))));
  ...
  // Messages sent by this system through the current DWS account (fallback
  // failure notifications) come back in the realtime stream with the sender
  // set to the login user. Feeding them back into the Agent would loop forever.
  if (/^AI 处理失败：/u.test(content) || /^AI 能力开启失败：/u.test(content)) return true;
  return false;
}
...
async function handleBatch(
  ...
  const processingTitle = () => {
    ...
    const elapsed = getDashboardConfig().showElapsed ? ` ${formatElapsed()}` : "";
  ...
  try {
    ...
    stopCardUpdates = () => {
      ...
      if (pendingCardTimer) clearTimeout(pendingCardTimer);
    ...
    const pumpCardUpdate = () => {
      ...
      // Read the setting for every update. A Web change therefore affects an
      // already running task, not only the next task.
      const remaining = getCardIntervalMs() - (Date.now() - lastCardUpdateAt);
      if (remaining > 0) {
        if (!pendingCardTimer) {
    ...
    const updateCard = (title: string, content: string) => {
      ...
      if (responseMode !== "card" || !activeCard) return;
    ...
    // Pausing is an intentional user action. Do not send the generic failure
    // notification through either the configured robot webhook or the current
    // DWS login account; the card is finalized below with a paused status.
    if (!queue?.paused) notifyGroupFailure(groupId, "Agent 执行异常", err, getDashboardConfig());
    if (elapsedTimer) {
      clearInterval(elapsedTimer);
      elapsedTimer = undefined;
    }
    liveReplies.delete(groupId);
    const message = err instanceof Error ? err.message : String(err);
    // Aborting a running Agent for an explicit pause commonly ends the child
    // process with SIGTERM's conventional exit code 143. This is expected
    // control flow, not an Agent failure, so keep it out of the error log.
    if (queue?.paused) log.info(`batch size=${events.length} paused by user`);
    else log.error(`batch size=${events.length} failed: ${message}`);
    if (card) {
      const stopped = queue?.paused === true;
      const stoppedContent = latestVisibleContent;
      // Cancel the interval timer and all delayed updates before the final
      ...
      await sendRobotText(groupId, getDashboardConfig(), `${label} 处理失败：${message.slice(0, 2_000)}`)
        .catch((sendErr) => log.warn(`text failure reply skipped: ${String(sendErr)}`));
    }
  }
}
...
async function enqueueGroupEvent(
  ...
  // A pause only stops the task that was running when the command arrived.
  // When a new matching message arrives and the queue is idle, clear the
  // paused flag so the next message can start a fresh task.
  if (!queue.running) queue.paused = false;
  if (queue.running) {
    const content = (event.content || event.text || "").trim();
  ...
  try {
    ...
    if (queue.paused) queue.pending.splice(0);
    queue.running = false;
...
function startGroupListener(
  ...
  const args = [
    "event", "+listen-im", "--kind", "all-group", "--events", "message",
    "--format", "ndjson",
  ];
  ...
  const acceptEvent = (event: DwsMessageEvent, options: { commandsOnly?: boolean } = {}) => {
    ...
    // A bot acknowledgement may be returned by history before the sender
    ...
    const bindingCommand = parseBindingCommand(rawContent, config.commandKeywords);
    ...
    // The direct binding path is only for commands that explicitly target a
    // mentioned member. A plain "打开 ai" must continue to the normal command
    // path below, where the robot is checked/added before the acknowledgement
    // is sent. Previously the broad bindingCommand check intercepted plain
    // commands and tried to send the acknowledgement before adding the bot.
    if (bindingCommand && authorizedOperator && mentionValues(event).length > 0) {
      const mentions = mentionValues(event);
      void monitorCommandChain.then(async () => {
        const members = mentions.length > 0 ? await listGroupMembers(groupId) : [];
        const targetMember = mentions.length > 0 ? resolveMentionedMember(members, mentions) : undefined;
        const targetSenderId = targetMember?.senderId || getSenderId(event);
        const targetSenderName = targetMember?.senderName || senderDisplayName(event) || getSenderId(event);
        if (!targetSenderId || targetSenderId === "unknown") return;
        await handleMentionMonitorCommand(bindingCommand, event, groupId, getDashboardConfig(), {
          groupId,
          groupName: await resolveCommandGroupName(groupId, getDashboardConfig()),
          senderId: targetSenderId,
          senderName: targetSenderName,
        }, updateDashboardConfig);
      }).catch((err) => log.error(`binding command failed: ${String(err)}`));
      return;
    }
    ...
    if (rawContent === "/new") {
      ...
      if (previous && now - previous < commandDeduplicationMs) return;
    ...
    if (command) {
      ...
      monitorCommandChain = monitorCommandChain
        .then(async () => {
          ...
          let hasRobot = await groupHasConfiguredRobot(groupId, getDashboardConfig());
          if (requiresGroupRobot && !hasRobot) {
            if (command === "open") {
              ...
              // `groupId` came from eventGroupId(event), which is the current
              // event's openConversationId. Never substitute a configured
              // group name, a cached conversation, or the DWS login user's ID.
              const openConversationId = groupId;
              const addBotArgs = ["chat", "+chat-add-bot", "--robot-code", robotCode, "--id", openConversationId, "--yes"];
              const shellQuote = (value: string) => /[^A-Za-z0-9_./:=+-]/.test(value) ? `'${value.replace(/'/g, "'\\''")}'` : value;
              log.info(`adding configured bot command=${[dwsPath, ...addBotArgs].map(shellQuote).join(" ")}`);
              log.info(`adding configured bot openConversationId=${openConversationId} robotCode=${robotCode}`);
              try {
                await addBotToGroup(openConversationId, robotCode);
                // The add operation succeeded for this exact group. Keep the
                // local result in sync so the following acknowledgement is
                // sent through the robot to the same openConversationId.
                hasRobot = true;
                log.info(`configured bot added to group openConversationId=${openConversationId}`);
              } catch (err) {
                const reason = err instanceof Error ? err.message : String(err);
                // robotCode is intentionally included in the diagnostic log so
                // an invalid bot identifier can be compared with the value in
                // the dashboard. This is the configured Client ID, not the
                // robotSenderOpenDingTalkId.
                log.error(`configured bot add failed group=${groupId} robotCode=${robotCode}: ${reason}`);
        ...
        .catch((err) => { log.error(`monitor command failed: ${String(err)}`); notifyGroupFailure(groupId, "群控制指令执行异常", err, getDashboardConfig()); });
      return;
    }
    if (!acceptsTarget(event, config)) {
    ...
    const matchedTarget = config.targets.find((target) => target.groupId === groupId && target.senderId === getSenderId(event));
    log.info(`group message matched monitor binding group=${groupId} groupName=${matchedTarget?.groupName || "<unknown>"} sender=${getSenderId(event)} senderName=${senderDisplayName(event) || matchedTarget?.senderName || "<unknown>"} content=${JSON.stringify(rawContent.slice(0, 300))} event=${event.event_id || event.message_id || event.messageId || "unknown"}`);
    void enqueueGroupEvent(event, groupId, queues, cardState, sessions, cardClient, getDashboardConfig, replies, liveReplies, runtime)
      .catch((err) => { log.error(String(err)); notifyGroupFailure(groupId, "群消息入队异常", err, getDashboardConfig()); });
  };
  ...
  // Pull the current DWS account's own group messages to compensate for
  // messages that may be missing from the realtime event stream.
  const stopPersonalHistoryPolling = startPersonalHistoryPolling({
...
async function main(): Promise<void> {
  ...
  // The dashboard is a separate process. Reload its file so changes made in
  // the web UI take effect in the listener without requiring a restart.
  const configReloadTimer = setInterval(() => {
    void loadDashboardConfig().then((latest) => {
      if (JSON.stringify(latest) === JSON.stringify(dashboardConfig)) return;
